...
2. В Amvera задайте обязательные переменные окружения Strapi:
   - `NODE_ENV=production`
   - `APP_KEYS`
   - `API_TOKEN_SALT`
   - `ADMIN_JWT_SECRET`
   ...
   - `JWT_SECRET`
   - `ENCRYPTION_KEY`
   - переменные БД (`DATABASE_*`) под целевое окружение
3. Убедитесь, что сборка выполняется командами из Strapi-проекта (`npm ci`, `npm run build`, `npm run start`).
4. После деплоя проверьте публичные endpoints:
   - `/api/hero?populate=*`
...
- [x] Новый репозиторий `strapi-cms` создан и заполнен через `git subtree split`
...
- [x] `strapi-cms` удален из текущего репозитория
- [x] CI текущего репозитория frontend-only
