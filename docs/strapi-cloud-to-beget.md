...
В репозитории в `strapi-cms` уже добавлен драйвер **`mysql2`**. При развёртывании на сервере выполняйте **`npm ci`** в каталоге проекта.
...
4. В каталоге, который соответствует **сайту поддомена CMS** (см. путь в панели к файлам сайта), загрузите содержимое **`strapi-cms/`** из репозитория (git clone, rsync, архив + распаковка). Удобно иметь структуру вида:

   ```text
   ~/admin.pantera-boxing.ru/strapi-cms/   # корень приложения Strapi
   ```

### 4.4. Переменные окружения Strapi (файл `.env`)

Создайте **`.env`** в корне `strapi-cms` на сервере (не коммитьте секреты). Пример — подставьте свои значения:
...
- [ ] В `strapi-cms` есть `mysql2`, в `.env` указано `DATABASE_CLIENT=mysql`.
